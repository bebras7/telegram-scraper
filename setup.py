...
def requirements():
	def csv_lib():
		...
		os.system("""
			...
			""")
	banner()
	print(gr+'['+cy+'+'+gr+']'+cy+' csv merge დასჭირდება 10 წუთი.')
	input_csv = input(gr+'['+cy+'+'+gr+']'+cy+' ჩართვა (y/n): ').lower()
	if input_csv == "y":
		csv_lib()
	else:
		pass
	print(gr+"[+] საჭირო ფაილების დაინსტალირება ...")
	os.system("""
...
def config_setup():
	...
	xid = input(gr+"[+] შეიყვანე API : "+re)
	cpass.set('cred', 'id', xid)
	xhash = input(gr+"[+] შეიყვანე ჰეშ ID : "+re)
	cpass.set('cred', 'hash', xhash)
	xphone = input(gr+"[+] მობილურის ნომერი : "+re)
	cpass.set('cred', 'phone', xphone)
	setup = open('config.data', 'w')
	cpass.write(setup)
	setup.close()
	print(gr+"[+] წარმატებით დაინსტალირდა !")
...
try:
	...
	print('\n'+gr+'['+re+'!'+gr+']'+cy+' no argument given : '+ sys.argv[0])
	print(gr+'['+re+'!'+gr+']'+cy+' for help use : ')
